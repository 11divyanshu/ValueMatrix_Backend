...
export const getUserNotification = async (request, response) => {
  try {
    Notification.find({$or :[{forAll: true, hideFrom:{"$ne": request.body.user_id}}, {forAll: false, user_type:"User",hideFrom:{"$ne":request.body.user_id}},]}, function(err, res){
        if(res){
            response.status(200).json({"notifications":res});
        }
        else
          response.json({"Error":"No Notification Found"});
    }
    );
  } catch (error) {
    console.log("Error : ", error);
